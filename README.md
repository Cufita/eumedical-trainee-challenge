...
**Tests (antes: 1 solo test de smoke; ahora: 27 archivos / 69 tests con `npm run test`, más 2 tests end-to-end con `npm run test:e2e`; ~97% de cobertura de líneas).**
...
- Una segunda ronda cerró los huecos restantes: navegación a Estudios/Perfil/Soporte desde el sidebar (antes las rutas lazy de esas tres pantallas nunca se invocaban en un test), el listener `languageChanged` de `src/i18n/index.ts` (actualiza `<html lang>` y persiste el idioma — nadie llamaba `i18n.changeLanguage()` en un test), y los botones "Cancelar" (Dashboard), "Ver detalles" (Recetas) y "Nueva consulta" (Consultas). Probar la navegación a Estudios expuso un bug real preexistente: `PatientCareVideoCard.tsx` llamaba `video.play().catch(...)`, pero `play()` puede devolver `undefined` en vez de una Promise (pasa en jsdom, y en algunos navegadores reales) — `.catch` sobre `undefined` rompía. Lo arreglé con `video.play()?.catch(...)`. Nota aparte: probé también volver de `/paciente` al sitio público, pero descarté ese test — al montarse `SitePage` completo (globo 3D, carruseles, lightbox) el universo de cobertura crece de golpe con código de marketing nunca antes medido, y el % total cae de ~97% a ~58% sin que sea una regresión real. Cobertura de líneas actual: ~97%.
