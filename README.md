...
**Performance — code splitting por ruta.** `npm run build` avisaba que todo el JS salía en un solo chunk de ~1.1MB minificado. Convertí las rutas de `App.tsx` a `React.lazy()` (sitio público, shell de paciente y cada una de sus 6 pantallas) envueltas en un `<Suspense>` con un fallback liviano (`RouteLoadingFallback.tsx`, reutiliza la misma animación de puntos del skeleton). Resultado: un chunk compartido de vendor (~336kB: React/React Router/etc.) y el área de paciente ahora se parte en piezas de 2-22kB por pantalla en vez de ir todo junto. El chunk del sitio público (`SitePage`, ~617kB) sigue arriba del umbral de 500kB de Vite — es contenido real de una sola página con scroll continuo (i18n con dos idiomas completos, video, testimonios, mapa), y partirlo más (por ejemplo, lazy-loading de secciones que igual son visibles al hacer scroll) cambiaría un número de warning por jank real de carga; no me pareció un buen trade para esta entrega, así que lo dejo documentado en vez de perseguir el número.

**SEO.** Usé `https://www.eumedical.es/` (el dominio real de la compañía, dado en el propio enunciado del reto) como canonical, URL de Open Graph y única entrada de `sitemap.xml` — no es un dominio inventado. Lo que sí falta y no fabriqué es un `og:image`: no hay ninguna foto real de Eumedical entregada para usarla como imagen de share (mismo criterio que ya regía el resto de las imágenes, ver `PRODUCT.md`), así que lo dejé sin ese tag en vez de improvisar una imagen que no representa a la marca. Además: `robots.txt` (permite `/`, bloquea `/paciente` por ser área privada de paciente), y sincronicé `document.documentElement.lang` con el idioma activo del switch ES/EN (antes quedaba fijo en `"es"` del `index.html` aunque el contenido pasara a inglés). No agregué meta description dinámica por ruta dentro de `/paciente`: es un área privada que `robots.txt` ya excluye de indexación, así que no tiene beneficio real de SEO, a diferencia del `<title>` por ruta (sección anterior), que sí ayuda a quien navega con varias pestañas abiertas.

**Con más tiempo, seguiría por acá** (además de lo ya listado en la sección 4): subir la cobertura de tests más allá de las páginas/lógica más ramificadas (falta `ProfilePage`, `SupportPage`, `PrescriptionsPage`), un pipeline de CI (no hay `.github/workflows`; hoy solo hay un hook de pre-commit local con Husky) que corra `type-check`, `lint` y `test` en cada push, y — si el chunk de `SitePage` se vuelve un problema real de performance medido (no solo el warning de build) — separar por `manualChunks` los datos de i18n (`en.ts`/`es.ts`) del resto, ya que ahí vive buena parte de esos ~617kB.
