...
// <canvas> fill/stroke styles can't resolve CSS custom properties on their
// own, so brand colors used inside Canvas 2D draw calls are read from
// tokens.css at runtime instead of being hardcoded here a second time.
...
function makeOceanTexture() {
  ...
  ocean.addColorStop(0, cssVar("--color-globe-ocean-dark"));
  ocean.addColorStop(0.5, cssVar("--color-globe-ocean-mid"));
  ocean.addColorStop(1, cssVar("--color-globe-ocean-dark"));
  ctx.fillStyle = ocean;
...
export function Globe() {
  ...
  useEffect(() => {
    ...
    const drawHighlight = (country: Country) => {
      ...
      hlCtx.fillStyle = cssVar("--color-gold");
    ...
    void fetch("/countries.json", { signal: abort.signal })
      ...
      .then((list) => {
        ...
        countries.forEach((country) => {
          borderCtx.fillStyle = cssVar("--color-globe-land");
  ...
  return (
    ...
    >
      <div className="absolute -left-5 bottom-[18%] z-[2] min-w-[108px] rounded-[18px] bg-navy px-6 py-4 text-center text-white shadow-[0_18px_38px_rgba(30,72,101,.28)] max-sm:left-0">
        ...
        <div className="mt-1.5 text-[14.5px] font-semibold tracking-[0.05em] text-white/90">
          {t("globe.countries")}
        </div>
      </div>
