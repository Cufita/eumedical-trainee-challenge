...
export function ServicesOrganism() {
  ...
  const patientCareSlides: PatientCareSlide[] = useMemo(
    () =>
      patientCareSlideAssets.map((asset) => ({
        ...asset,
        title: t(`services.slides.${asset.id}.title`),
        description: t(`services.slides.${asset.id}.description`),
      })),
    [t],
  );
  ...
  return (
    <section id="eu-servicios" className="bg-navy px-(--edge) py-20 text-white md:py-24">
      <Container>
        <div className="mx-auto max-w-[40em] text-center">
          <SectionEyebrow light>{t("services.eyebrow")}</SectionEyebrow>
          <h2 className="mx-auto mt-3 max-w-[18em] text-[clamp(1.9rem,3.6vw,2.75rem)] leading-[1.14] text-white">
            {t("services.title")}
          </h2>
          <p className="mx-auto mt-4 max-w-[52ch] text-[15.5px] leading-[1.62] text-white/80">
  ...
  return (
    <div ref={ref} className="patient-care-fade" data-visible={visible}>
      ...
      <div className="mt-6 text-center">
        ...
        <p className="mx-auto mt-2 max-w-[46ch] text-[15px] leading-[1.5] text-white/80">{slide.description}</p>
      </div>
    </div>
  );
}
